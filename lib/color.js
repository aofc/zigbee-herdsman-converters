...
function syncColorState(newState, oldState, options) {
    ...
    case 'xy':
        ...
        if (newState.hasOwnProperty('color') && newState.color.hasOwnProperty('hex')) {
            Object.assign(result.color, {'hex': newState.color.hex});
        }
        if (result.color.hasOwnProperty('hex')){
            result = result;
        } else if (result.color.hasOwnProperty('x') && result.color.hasOwnProperty('y')) {
